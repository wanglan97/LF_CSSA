...
from model.model_recognition import Model
from config.config_wav2vec import Config
from load_data_wav2vec import MMdataloader
    ...
    while True:
        ...
        with tqdm(dataloader['train']) as td:
            for batch_data in td:
                ...
                # compute_loss
                loss = criterion(a_out,labels)+ criterion(t_out,labels)
                # backward
                loss.backward()
                # update
                optimizer.step()
        ...
        # validation
        val_result, val_loss = test(model, dataloader['valid'], mode='VAL')
        ...
        if val_loss < best_valid:
            ...
            best_valid=val_loss
            model_path = os.path.join('../results/model_save\\', 'audio.pth')
    ...
    with torch.no_grad():
        with tqdm(dataloader) as td:
            for batch_data in td:
                ...
                t_out,a_out= model(text, audio)
                loss = criterion(t_out, labels) + criterion(a_out, labels)
                eval_loss += loss.item()
                y_pred.append(a_out)
                y_true.append(labels)
...
def run(config):
    ...
    pretrained_path = os.path.join('../results/model_save\\', 'audio.pth')
    assert os.path.exists(pretrained_path)
    model.load_state_dict(torch.load(pretrained_path))
    # do test
    if config.debug_mode:
        # using valid dataset to debug hyper parameters
        results,t= test(model, dataloader['valid'], mode="VAL")
    else:
        results,t = test(model, dataloader['test'], mode="TEST")
...
def run_debug(seeds, debug_times=50):
    ...
    for i in range(debug_times):
        ...
        # load results file
        save_path = os.path.join(r'../results/result_save\\', 'audio-wav2vec-debug.csv')
...
def run_normal(seeds):
    ...
    save_path=os.path.join(r'../results/result_save\\', 'audio_wav2vec.csv')
    if not os.path.exists(r'../results/result_save'):
        os.makedirs(r'../results/result_save')
    df.to_csv(save_path, index=None)
    print('Results are saved to %s...' % (save_path))
if __name__=='__main__':
    ...
    # print(seeds)
    run_normal(seeds)
    # run_debug(seeds,debug_times=150)
