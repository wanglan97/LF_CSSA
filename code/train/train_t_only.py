...
from model.model_t_only import Model
from config.config_t_only import Config
...
def train(model,dataloader,config):
    ...
    while True:
        ...
        if val_loss < best_valid:
            ...
            best_valid=val_loss
            model_path = os.path.join('../results/model_save\\', 'audio_t.pth')
...
def run(config):
    ...
    pretrained_path = os.path.join('../results/model_save\\', 'audio_t.pth')
...
def run_debug(seeds, debug_times=50):
    ...
    for i in range(debug_times):
        ...
        # load results file
        save_path = os.path.join(r'../results/result_save\\', 'audio-t-debug.csv')
...
def run_normal(seeds):
    ...
    save_path=os.path.join(r'../results/result_save\\', 'audio_t.csv')
    if not os.path.exists(r'../results/result_save'):
        os.makedirs(r'../results/result_save')
    df.to_csv(save_path, index=None)
    print('Results are saved to %s...' % (save_path))
