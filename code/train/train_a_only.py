...
from model.model_a_only import Model
from config.config_a_only import Config
from load_data_a_t import MMdataloader
...
def train(model,dataloader,config):
    ...
    while True:
        ...
        val_acc1 = val_result['M']['MAE']
        train_total_acc.append(train_results['Corr'])
        valid_total_acc.append(val_acc1)
        ...
        if val_loss < best_valid:
            ...
            best_valid=val_loss
            model_path = os.path.join('../results/model_save\\', 'audio_a.pth')
...
def run(config):
    ...
    pretrained_path = os.path.join('../results/model_save\\', 'audio_a.pth')
...
def run_debug(seeds, debug_times=50):
    ...
    for i in range(debug_times):
        ...
        # load results file
        save_path = os.path.join(r'../results/result_save\\', 'audio-a-debug.csv')
...
def run_normal(seeds):
    ...
    save_path=os.path.join(r'../results/result_save\\', 'audio_a.csv')
    if not os.path.exists(r'../results/result_save'):
        os.makedirs(r'../results/result_save')
    df.to_csv(save_path, index=None)
    print('Results are saved to %s...' % (save_path))
