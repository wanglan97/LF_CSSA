...
from config.config_a_t import Config
...
def train(model,dataloader,config):
    def adjust_learning_rate(optimizer, epoch, config):
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
    optimizer = optim.Adam([{"params": list(model.parameters())[:3], "lr": config.factor_lr},
                                {"params": list(model.parameters())[5:], "lr": config.lr}],
    ...
    while True:
        ...
        if val_loss < best_valid:
            ...
            best_valid=val_loss
            model_path = os.path.join('../results/model_save\\', 'LMF.pth')
...
def run(config):
    ...
    model=LMF(config).to(device)
    print("#"*40)

    #do train
    train(model,dataloader,config)
    # load pretrained model
    pretrained_path = os.path.join('../results/model_save\\', 'LMF.pth')
...
def run_debug(seeds, debug_times=50):
    ...
    for i in range(debug_times):
        ...
        # load results file
        save_path = os.path.join(r'../results/result_save\\', 'LMF-debug.csv')
...
def run_normal(seeds):
    ...
    save_path=os.path.join(r'../results/result_save\\', 'LMF.csv')
    if not os.path.exists(r'../results/result_save'):
        os.makedirs(r'../results/result_save')
    df.to_csv(save_path, index=None)
    print('Results are saved to %s...' % (save_path))
