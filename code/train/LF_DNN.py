...
from model.LF_DNN import LF_DNN
...
def train(model,dataloader,config):
    ...
    while True:
        ...
        if val_loss < best_valid:
            ...
            best_valid=val_loss
            model_path = os.path.join('../results/model_save\\', 'LF_DNN.pth')
...
def run(config):
    ...
    model=LF_DNN(config).to(device)
    print("#"*40)

    #do train
    train(model,dataloader,config)
    # load pretrained model
    pretrained_path = os.path.join('../results/model_save\\', 'LF_DNN.pth')
...
def run_debug(seeds, debug_times=50):
    ...
    for i in range(debug_times):
        ...
        # load results file
        save_path = os.path.join(r'../results/result_save\\', 'LF_DNN-debug.csv')
...
def run_normal(seeds):
    ...
    save_path=os.path.join(r'../results/result_save\\', 'LF_DNN.csv')
    if not os.path.exists(r'../results/result_save'):
        os.makedirs(r'../results/result_save')
    df.to_csv(save_path, index=None)
    print('Results are saved to %s...' % (save_path))
