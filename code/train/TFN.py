...
def train(model,dataloader,config):
    ...
    while True:
        ...
        if val_loss < best_valid:
            ...
            best_valid=val_loss
            model_path = os.path.join('../results/model_save\\', 'TFN.pth')
...
def run(config):
    ...
    model=TFN(config).to(device)
    print("#"*40)

    #do train
    train(model,dataloader,config)
    # load pretrained model
    pretrained_path = os.path.join('../results/model_save\\', 'TFN.pth')
...
def run_debug(seeds, debug_times=50):
    ...
    for i in range(debug_times):
        ...
        # load results file
        save_path = os.path.join(r'../results/result_save\\', 'TFN-debug.csv')
...
def run_normal(seeds):
    ...
    save_path=os.path.join(r'../results/result_save\\', 'TFN.csv')
    if not os.path.exists(r'../results/result_save'):
        os.makedirs(r'../results/result_save')
    df.to_csv(save_path, index=None)
    print('Results are saved to %s...' % (save_path))
