...
for index, y in enumerate(list_name):
    ...
    # 并且求取平均值,修改了spectrum.py中的n_fft=1024
    f0 = np.mean(librosa.feature.zero_crossing_rate(y=X, hop_length=512).T, axis=0)  # (seq_len, 1),计算音频时间序列的过零率。
    cqt = np.mean(librosa.feature.chroma_cqt(y=X, sr=22050 * 2, hop_length=512).T, axis=0)  # (seq_len, 12)
    ...
    # print('feature',feature)
    df3.loc[bookmark] = [feature]
    bookmark = bookmark + 1
df3 = pd.DataFrame(df3['feature'].values.tolist())

print("文本特征维度", df1.shape)
print("拼音特征维度", df2.shape)
print("音频特征维度", df3.shape)
# print('df2', df2)
# 获取音频特征取值
...
# print('df3',df3)
# print(df4)
# 音频特征、文本特征和目标特征合并,df1是文本300、df2拼音300、df3是音频229
newdf = pd.concat([df1, df2, df3, labels], axis=1)
df4 = pd.DataFrame(newdf.values.tolist())
# print('df3',df3)
# 将旧的名字label转换成新的名字，0
# rnewdf = newdf.rename(index=str, columns={"0": "label"})
# print(newdf[:])
print("拼接后特征维度", df4.shape)#1200,830
...
# 获取训练特征
trainfeatures = train.iloc[:, :-1]#955,829
print('trainfeatures', trainfeatures.shape)
audio_train = train.iloc[:, 600:829]
print('audio_train', audio_train.shape)#955,229
text_train = train.iloc[:, 0:300]
print('text_train', text_train.shape)#955,300
pyT_train = train.iloc[:, 300:600]#955,300
print('pyT_train', pyT_train.shape)
# print(audiofeatures)
...
audio_test = test.iloc[:, 600:829]
pyT_test = train.iloc[:, 300:600]
text_test = test.iloc[:, 0:300]
# 获取测试标签
testlabel = test.iloc[:, -1:]
print('训练集样本数：', len(trainfeatures))#955
print('测试集样本数：', len(testfeatures))#245
...
output = open('../data/dataWithPYT.pkl', 'wb')
