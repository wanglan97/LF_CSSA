...
class Graph_MFN(nn.Module):
	def __init__(self, args):
		...
		# print(args)
		self.d_l, self.d_a = args.feature_dim
		self.dh_l, self.dh_a= args.hidden_dim
		total_h_dim = self.dh_l + self.dh_a
		self.mem_dim = args.memsize
		self.inner_node_dim = args.inner_node_dim
		self.singleton_l_size, self.singleton_a_size= args.hidden_dim
		...
		pattern_model = nn.Sequential(nn.Linear(100, self.inner_node_dim)).to(device)
		efficacy_model = nn.Sequential(nn.Linear(100, self.inner_node_dim)).to(device) # Note : actually here inner_node_dim can change arbitrarily
		self.graph_mfn = DynamicFusionGraph(pattern_model, [self.singleton_l_size, self.singleton_a_size], self.inner_node_dim, efficacy_model, device).to(device)
	...
	def forward(self, text_x, audio_x):
		...
		for i in range(t):
			...
			attended, _, efficacies = self.graph_mfn([l_singleton_input, a_singleton_input])
