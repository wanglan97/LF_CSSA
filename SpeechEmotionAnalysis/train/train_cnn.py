...
def train(model, dataloader):
    ...
    for s in range(1000):
        ...
        # validation
        val_acc, val_f1, val_loss = test(model, test_loader)
        ...
        if val_acc > best_acc:
            ...
            model_path = os.path.join('results/model_save\\', 'emotion.pth'.format(best_epoch))
...
if __name__ == '__main__':
    ...
    x_traincnn=data['x_train']
    y_train=data['y_train']
    x_testcnn=data['x_test']
    y_test=data['y_test']
    pkl_file.close()
    train_dataset = Data.TensorDataset(torch.tensor(x_traincnn, dtype=torch.float32),
                                       torch.tensor(y_train, dtype=torch.float32))
    train_loader = Data.DataLoader(dataset=train_dataset, batch_size=128, shuffle=False)
    test_dataset = Data.TensorDataset(torch.tensor(x_testcnn, dtype=torch.float32),
...
#acc 0.3574660633484163 f1 0.3561181047262367

#acc 0.47058823529411764 f1 0.47375220686362374   969轮

#acc 0.4796380090497738 f1 0.47990091909407434     1927轮


# 0.497737556561086 f1 0.49741168783181855   3000轮


# acc 0.5113122171945701 f1 0.5125099526005671  1000轮